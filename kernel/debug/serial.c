...
int serial_received(uintptr_t port)
{
    return inb(port + SERIAL_REG_LSR) & 1;
}

/* Check whether the serial port is idle */
int is_transmit_empty(uintptr_t port)
{
    return inb(port + SERIAL_REG_LSR) & 0x20;
}

/* Read serial port */
uint8_t read_serial(uintptr_t port)
{
...
/* Write serial port */
void write_serial(uintptr_t port, uint8_t data)
{
...
uint8_t get_serial_status(uintptr_t port)
{
...
static void uart_write_reg(int reg, uint8_t val) {
    ...
    volatile void *serial_base = (volatile void*)(hhdm_offset + 0x1fe001e0);
