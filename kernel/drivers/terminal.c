...
static void setup_cpos_default() {
  TerminalPalette palette = {.background = 0x000000,
                             .foreground = 0xffffff,
...
void init_terminal() { //有点问题(((
  // TerminalDisplay display = {.width   = framebuffer->width,
  //         .height  = framebuffer->height,
  //         .address = framebuffer->address};
  // volatile float size = 10.0f * ((volatile float) framebuffer->width / 1024);
  ...
  //        cpu_hlt;
  //    }
  // terminal_init(&display, size, malloc, free, printks);
