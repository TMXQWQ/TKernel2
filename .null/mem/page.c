...
/* 页错误处理 */
// __attribute__((target("no-sse,no-sse2,no-fp")))
__attribute__((interrupt)) static void page_fault_handle(struct interrupt_frame_t *frame, uint64_t error_code)
{
	// __asm__("hlt");	//debug:看日志
...
// }
void switch_page_directory(pcb_t *target)
{
	uint64_t cr3 = target->cr3;
	current_directory = target->page_dir;
	__asm__ __volatile__("mov %0, %%cr3" :: "b"(cr3));
}
